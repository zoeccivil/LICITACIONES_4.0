...
los inserta masivamente en MySQL con las conversiones de tipo apropiadas.
"""
...
def convert_sqlite_value(value: Any, column_type: str) -> Any:
    """
    Convertir valor SQLite a valor compatible con MySQL.
        ...
        if isinstance(value, str):
            ...
            for fmt in [
                ...
                try:
                    ...
                    continue
            # If no format matches, return as-is (MySQL may be able to parse it)
            return value
        return value
...
def insert_data_bulk(session, table_name: str, rows: List[Dict[str, Any]]) -> int:
    """
    Insertar datos masivamente en tabla MySQL.
...
def migrate_table(sqlite_conn: sqlite3.Connection, table_name: str) -> Dict[str, Any]:
    """
    Migrar una sola tabla de SQLite a MySQL.
...
def verify_migration(sqlite_conn: sqlite3.Connection) -> Dict[str, Tuple[int, int]]:
    """
    Verificar que los conteos de filas coincidan entre SQLite y MySQL.
    
    Args:
        sqlite_conn: SQLite connection
        
    Returns:
        Dictionary mapping table_name -> (sqlite_count, mysql_count)
    """
    results = {}
...
def main():
    ...
    engine = get_engine()
    if engine is None:
        print("Error: MySQL engine not initialized.")
        print("Please check your .env file and database configuration.")
        return 1
    
    print("=" * 70)
    print("Migración de Datos MySQL")
    print("=" * 70)
    print(f"Source (SQLite): {sqlite_path}")
    print(f"Target (MySQL): {os.getenv('DB_HOST')}:{os.getenv('DB_PORT')}/{os.getenv('DB_NAME')}")
    print("=" * 70)
    print()
    ...
    if not args.skip_verification and not failed_tables:
        ...
        print(f"{'Table':<30} {'SQLite':>10} {'MySQL':>12} {'Status':>10}")
        print("-" * 70)
