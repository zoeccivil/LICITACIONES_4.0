...
ACCENT = QColor("#8B5CF6")        # amethyst
ACCENT_ALT = QColor("#A78BFA")
BG_WINDOW = QColor("#1F1A24")     # violeta carbón (no negro)
BG_BASE = QColor("#231C29")
BG_ALT = QColor("#271F2E")
FG_TEXT = QColor("#EFE7F5")
FG_TEXT_SEC = QColor("#C9BFE0")
BORDER = QColor("#3B2F45")
DISABLED_FG = QColor("#9589A8")
LINK = QColor("#A78BFA")
VISITED = QColor("#C084FC")

def apply_amethyst_dim_theme(app: QApplication) -> None:
    app.setStyle("Fusion")
    pal = QPalette()
    ...
    pal.setColor(QPalette.ColorRole.ToolTipBase, QColor("#2A2232"))
    ...
    pal.setColor(QPalette.ColorGroup.Disabled, QPalette.ColorRole.Highlight, QColor("#3A2E47"))
    pal.setColor(QPalette.ColorGroup.Disabled, QPalette.ColorRole.HighlightedText, QColor("#B7A9D1"))

    app.setPalette(pal)

    app.setStyleSheet(f"""
        QWidget {{ color: {FG_TEXT.name()}; background-color: {BG_WINDOW.name()}; }}

        QMenuBar {{ background: {BG_WINDOW.name()}; border-bottom: 1px solid {BORDER.name()}; }}
        QMenuBar::item {{ padding: 6px 10px; }}
        QMenuBar::item:selected {{ background: {BG_ALT.name()}; border-radius: 4px; }}

        QMenu {{ background: {BG_BASE.name()}; border: 1px solid {BORDER.name()}; }}
        QMenu::item:selected {{ background: {ACCENT.name()}22; }}

        QToolBar {{ background: {BG_WINDOW.name()}; border-bottom: 1px solid {BORDER.name()}; }}
        ...
        QProgressBar::chunk {{ background: {ACCENT.name()}; border-radius: 6px; }}
